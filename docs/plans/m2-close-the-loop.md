...
`fg-norn-experimental/docs/design/PLAN.md`, which solved the same
...
bugs, it finds nothing, loudly. This is the lesson `fg-norn`'s PLAN §0 paid
for — fg-spec's delta-presence oracle sat at a 77% noise floor on
...
3. Mechanize the known-divergence registry. ADR 0006 lists two open port
   defects by hand. Following `fg-norn` PLAN §3.2, make them data, where a
