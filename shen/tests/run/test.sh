#!/bin/sh
# Run-driver suite: the production scenario driver of ADR 0007 (D1 step
# loop, D2 pinned selections, D5 budget enforcement).
#
# Every available Shen port must reproduce the checked-in golden semantic
# output byte for byte. Driver runs are pure functions of the scenario
# declaration (seed, budget, roster, routes, properties) and the model
# table, so any per-port disagreement is a semantic bug.
#
...
if [ ! -f "$GOLDEN" ]; then
  printf 'run suite: FAIL missing %s\n' "$GOLDEN" >&2
...
if [ "$ran" -eq 0 ]; then
  printf 'run suite: BLOCKED no Shen port launcher was available\n' >&2
  exit 1
fi

cases=$(/usr/bin/grep -c '^RUN|' "$GOLDEN" 2>/dev/null || printf '0')
printf 'run suite: ALL PASS ports=%s lines=%s\n' "$ran" "$cases"
