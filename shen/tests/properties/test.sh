#!/bin/sh
# M1 Wave D suite: deterministic property verdicts (SPEC.md 18.1).
#
# Every required Shen port must reproduce the checked-in golden semantic
# output byte for byte. A verdict is a pure function of (canonical trace,
# property spec), so any per-port disagreement is a semantic bug, not a
# formatting one, and the exact-golden comparison is what says so.
#
...
GOLDEN=shen/tests/properties/golden.txt
...
SEMANTIC='^(PROP\||SLOT\||MUT\||LOAD\||TRACE\||FAIL\||PROPERTIES CASES: |PROPERTIES: FAIL$|ALL PASS$)'

work=$(mktemp -d "${TMPDIR:-/tmp}/urdr-properties.XXXXXX")
...
if [ "$ran" -eq 0 ]; then
  printf 'properties suite: BLOCKED no Shen port launcher was available\n' >&2
  exit 1
fi

cases=$(/usr/bin/grep -c '^\(PROP\|SLOT\|MUT\|LOAD\|TRACE\)|' "$GOLDEN")
printf 'properties suite: ALL PASS ports=%s cases=%s\n' "$ran" "$cases"
