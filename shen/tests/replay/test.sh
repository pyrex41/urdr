#!/bin/sh
# M1 Wave E suite: SPEC.md 20 event records, decision-transcript replay,
# and modeled-run certificates.
#
# Every required Shen port must reproduce the checked-in golden semantic
# output byte for byte. An event root, a run id, and a certificate are
# pure functions of their declared inputs, so any per-port disagreement
# is a semantic bug, not a formatting one, and the exact-golden
# comparison is what says so. The LOOP line additionally asserts that 100
# consecutive replays of a fixture run produce identical canonical roots
# WITHIN each port; the golden comparison asserts they agree ACROSS
# ports.
#
...
if [ "$ran" -eq 0 ]; then
  printf 'replay suite: BLOCKED no Shen port launcher was available\n' >&2
  exit 1
fi

cases=$(/usr/bin/grep -c \
  '^\(NODES\|CA\|EL\|RT\|E2E\|SIG\|DIV\|MUT\|MARK\|CERT\|LOOP\)|' "$GOLDEN")
printf 'replay suite: ALL PASS ports=%s lines=%s\n' "$ran" "$cases"
